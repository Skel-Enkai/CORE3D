...
void Model::traverseNode(unsigned int nextNode, glm::mat4 matrix)
{
  ...
  // glm::mat4 matNextNode = matNode * matrix;
	...
	// passing the previous Matrix so all transformations are applied down to all children
...
std::vector<GLuint> Model::getIndices(json accessor)
{
  ...
  else 
  {
	...
	{
		for (unsigned int i = beginningOfData; i < beginningOfData + count * 4;)
		{
			unsigned char bytes[] = { data[i++], data[i++], data[i++], data[i++] };
			unsigned int value;
	...
	{
		for (unsigned int i = beginningOfData; i < beginningOfData + count * 2;)
		{
